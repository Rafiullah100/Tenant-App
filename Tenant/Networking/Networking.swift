...
class Networking {
    ...
    func addProperty(route: Route, imageParameter: String, images: [UIImage], parameters: [String: Any], completion: @escaping (Result<AddPropertyModel, AppError>) -> Void) {
        ...
        .response { response in
            ...
            case .success:
                do {
                    let decodedResponse = try JSONDecoder().decode(AddPropertyModel.self, from: response.data ?? Data())
                    completion(.success(decodedResponse))
                } catch let decodingError {
                    print("Decoding error: \(decodingError)")
                    completion(.failure(AppError.unknownError))
    ...
    func updateProfile(route: Route, imageParameter: String, images: [UIImage], parameters: [String: Any], completion: @escaping (Result<UpdateProfileModel, AppError>) -> Void) {
        print(parameters, images.count)
        let urlStr = Route.baseUrl + route.description
        let headers: HTTPHeaders = [
            "Content-type": "multipart/form-data",
        ...
        .response { response in
            ...
            case .success:
                do {
                    let decodedResponse = try JSONDecoder().decode(UpdateProfileModel.self, from: response.data ?? Data())
                    completion(.success(decodedResponse))
                } catch let decodingError {
                    print("Decoding error: \(decodingError)")
                    completion(.failure(AppError.unknownError))
